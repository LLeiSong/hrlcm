...
then
	...
	echo "white-space delimited tile IDs can be given in place of a single tile ID ">&2
...
then
	...
	grep -v productlist_ $html > /tmp/tmp.no.productlist.$$  && mv /tmp/tmp.no.productlist.$$ $html 
