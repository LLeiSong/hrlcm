...
Before official running, test a bit about the largest values of `num_worker`, `train_batch_size`, `val_batch_size` that could be used. For `g5.12xlarge`, these values could be:
...
- 0.00001 is a good end point.

`final_lr` for `optim.AdaBound`:

- 0.01 still works fine.
...
```
python hrlcm/fit_compose_lr.py --exp_name unet_all_norm_200 --num_workers 48 --gpu_devices '0,1,2,3' --min_lr 0.000001 --max_lr 0.00001 --save_freq 20 --epochs 201
```
