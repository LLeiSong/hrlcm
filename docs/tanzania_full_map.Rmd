...
move_train <- lapply(1:nrow(north_train), function(n) {
    ...
    # Image, subset it
    things_to <- north_train[n, 'img']
    things_from <- gsub("train", "dl_train", things_to)
    things_to <- file.path(here("results/tanzania"), things_to)
    things_from <- file.path(here("results/north"), things_from)
    
    sat <- rast(things_from)
    sat <- subset(sat, var_selected$var)
    writeRaster(sat, things_to, overwrite = TRUE)
})

move_valid <- lapply(1:nrow(north_valid), function(n) {
    # Label, directly copy
    things_to <- north_valid[n, "label"]
...
- Using all bands with image augmentation including image standardization
- Using only NICFI bands with image augmentation including image standardization （* This provides option of prediction for other years, it is much faster to prepare satellite images).
