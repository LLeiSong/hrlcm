...
The scripts used for training U-Net are under directory hrlcm.

#### Preparation

**[Launch AWS instance]** a few instance could be used for different needs.
...
**[Clean and organize directories]** pull branch `full_country` of `hrlcm` to the instance, and create relative folders for dataset.

**[Parse dataset from Dropbox]** run script `hrlcm/utils/parse_dataset.sh` to parse dataset into the instance.

The instance should be ready to run training for now.

#### Training

Multiple type of models will be trained:
...
```
python hrlcm/fit_compose_lr.py --exp_name unet_clr_200epc --num_workers 32 --gpu_devices '0,1,2,3' --epochs 201
```

Evaluation:

```
python hrlcm/evaluate.py --args_path 'results/tanzania/unet_clr_200epc/checkpoints/args.pkl' --checkpoint_file 'results/tanzania/unet_clr_200epc/checkpoints/final.pth' --data_dir 'results/tanzania' --out_dir 'results/tanzania/evaluation' --stats_dir 'results/tanzania/norm_stats' --num_workers 32 --batch_size 32 --gpu_devices '0,1,2,3'

scp -r ubuntu@ec2-18-191-228-234.us-east-2.compute.amazonaws.com:~/hrlcm/results/evaluation /Users/pinot/Dropbox/research/hrlcm/results/tanzania/unet_clr_200epc
```

Prediction

```
python hrlcm/predict.py --args_path 'results/tanzania/unet_clr_200epc/checkpoints/args.pkl' --checkpoint_file 'results/tanzania/unet_clr_200epc/checkpoints/final.pth' --fname_predict 'dl_catalog_predict.csv' --data_dir 'results/tanzania' --out_dir 'results/tanzania/prediction' --stats_dir 'results/tanzania/norm_stats' --num_workers 32 --batch_size 32 --gpu_devices '0,1,2,3'

scp -r ubuntu@ec2-18-191-228-234.us-east-2.compute.amazonaws.com:~/hrlcm/results/prediction /Users/pinot/Dropbox/research/hrlcm/results/tanzania
```
