...
```{r}
# Get file names
fns <- list.files(here('results/scalars/single_compose_lr'), 
                  pattern = 'compose_lr',
                  full.names = T)
...
items <- c('AA', 'IoU', 'Cropland', 'Forest', 
...
# Get file names
fns <- list.files(here('results/scalars/single_step_lr'), 
                  pattern = 'step_lr',
                  full.names = T)
...
# Plot
aa <- merge(unet_compose_lr_scalars$AA, 
            unet_step_lr_scalars$AA, 
            by = 'Step') %>% 
