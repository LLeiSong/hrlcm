...
# Barplot
cls_freq <- data.frame(area = unlist(class_area) / whole_area * 100,
                       err = c(adjust_class_err / whole_area * 100, 0),
    ...
    mutate(err_adjust_prop = glue(
        paste0('{format(round(area, digits = 1), nsmall = 1)}',
               ...
               '%'))) %>% 
    arrange(-area) %>% 
    mutate(label_ypos = c(0.75, 0.75, 0.52, 2.9, 3, 3.8, 5.2, 12)* area)
cls_freq <- cls_freq %>% 
    mutate(landcover = factor(landcover, 
                              levels = cls_freq %>% 
                                  arrange(desc(area)) %>% 
...
ggsave(here('docs/figures/figure7_final_map.tiff'),
