...
**UN-FAO’s Land Cover Classification System (LCCS)**
```
...
grids_pure_count <- read.csv('data/intermid/pure_grids_count.csv',
                             stringsAsFactors = F)
...
Based on the table, the first three minor classes (`r grids_pure_count$name[1:3]`) have the similar number of pure grids. So we keep all grids from these classes, and then randomly select the average number (`r ceiling(mean(grids_pure_count$n[1:3]))`) of grids from other classes.

##### Reselect
```{r reselect}
grids_pure_count$n[4:nrow(grids_pure_count)] <- ceiling(mean(grids_pure_count$n[1:3]))
tiles_grids_selected <- do.call(rbind, 
                                lapply(grids_pure_count$name, 
                                       function(nm){
    message(nm)
    n_grids <- grids_pure_count %>% 
      filter(name == nm) %>% 
      pull(n)
    set.seed(123)
    tile_select <- tiles_labels %>% 
...
tiles_grids_valid <- tiles_grids_selected %>% 
  ...
  dplyr::select(tile, index, usage)
catalog_train_val <- rbind(tiles_grids_train, 
                               tiles_grids_valid)
write.csv(catalog_train_val, 
          'data/intermid/catalog_train_val_pure.csv',
          row.names = F)
```

#### [ARCHIVED] Get large amount of weak labels.
Based on the prior experiments, this method didn't do well. The train dataset is "under-representative" in terms of DL training (Validate loss curve is below the train loss significantly). This might be caused by either the super imbalance in both train and validate dataset, or the noises in the labels.
...
Select test dataset for final round.
...
catalog_train_val_test <- catalog_train_val
...
labels_path <- '/Volumes/elephant/labels_pure'

lapply(unique(catalog_train_val_test$tile), 
         function(tile_nm){
