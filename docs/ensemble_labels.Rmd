...
tiles <- do.call(rbind, mclapply(1:nrow(tiles), function(n){
  tile <- tiles %>% slice(n)
  tiles_grids <- st_make_grid(tile, n = c(8, 8)) %>% 
    st_sf() %>% arrange() %>% 
    mutate(tile = tile$tile,
                       index = c(1:64))
}, mc.cores = 10))
st_write(tiles, 'data/geoms/tiles_grids.geojson')
tiles <- tiles %>% vect()

tiles_labels <- do.call(bind_rows, mclapply(1:nrow(tiles), function(n){
  labels_each <- crop(lc_labels, tiles[n, ])
  freq(labels_each) %>% data.frame() %>% 
    filter(value != 0) %>%
...
tiles_grids_selected <- do.call(rbind, 
  ...
  if (nm %in% c('Forest', 'Grassland', 'Cropland', 'Shrubland')){
    tile_select <- tiles_labels %>% 
      ...
      mutate(group = ifelse(!!rlang::sym(nm) >= 0.8, 1, 
                            ifelse(!!rlang::sym(nm) >= 0.6, 2, 3))) 
    ns <- tile_select %>% group_by(group) %>% summarise(n = n()) %>% 
      mutate(n = ifelse(n < 1000, n, 1000))
    tile_select <- do.call(rbind, 
                           lapply(unique(tile_select$group), 
                                  function(g){
      set.seed(124 + g)
      tile_select %>% filter(group == g) %>% 
        sample_n(size = ns %>% 
                   filter(group == g) %>% 
                   pull(n))
    }))
...
tiles_grids_test <- tiles_grids_selected %>%
  ...
  mutate(usage = 'test') %>% 
  dplyr::select(tile, index, usage)
catalog_pred_val_test <- rbind(tiles_grids_train, 
                               tiles_grids_valid, 
                               tiles_grids_test)
write.csv(catalog_pred_val_test, 
          'data/intermid/catalog_pred_val_test.csv',
          row.names = T)
```

#### Generate labels images
```{r labels_img}
tiles <- st_read('data/geoms/tiles_grids.geojson',
                 quiet = T)
lc_labels <- rast('data/intermid/lc_labels_exd_val.tif')
...
lapply(unique(catalog_pred_val_test$tile), function(tile_nm){
  tiles_each <- catalog_pred_val_test %>% filter(tile == tile_nm)
  ...
  temp <- rast(nm)[[1]] %>% aggregate(fact = 512)
  vals <- matrix(1:64, nrow = 8, ncol = 8) %>% t()
  ...
  lapply(1:nrow(tiles_each), function(n){
    ...
    temp_each <- trim(temp_each) %>% 
      disaggregate(fact = 512)
