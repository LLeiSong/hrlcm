...
def get_compose_lr(model, epochs, min_lr=0.0001, max_lr=0.001, stage1=50, stage2=130, stage3=170):
    ...
    optimizer_lr = optim.AdaBound(model.parameters(), lr=max_lr, final_lr=0.0001, amsbound=True)
    lrs = []
    for i in range(epochs):
        optimizer_lr.step()
        if i <= stage1:
            if i == stage1:
                lr_scheduler_1 = torch.optim.lr_scheduler.CyclicLR(optimizer_lr, base_lr=max_lr - 0.000002,
                                                                   max_lr=max_lr + 0.000002,
            ...
            lr_scheduler_1.step()
            if i == stage2:
                lr_scheduler_2 = torch.optim.lr_scheduler.CyclicLR(optimizer_lr, base_lr=(max_lr - 0.000002) / 2,
                                                                   max_lr=(max_lr + 0.000002) / 2,
